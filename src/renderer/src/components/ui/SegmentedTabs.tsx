...
export function SegmentedTabs<T extends string>({ items, value, onChange, className }: SegmentedTabsProps<T>) {
  ...
  return (
    <div
      className={["relative inline-grid items-center gap-1 rounded-lg border border-slate-200 bg-white p-1 shadow-sm", className]
        .filter(Boolean)
        .join(" ")}
      ...
      {items.map((item) => (
        <button
          ...
          className={[
            ...
            value === item.value ? "text-white" : "text-slate-600 hover:text-slate-900"
          ].join(" ")}
        >
          {item.label}
        </button>
