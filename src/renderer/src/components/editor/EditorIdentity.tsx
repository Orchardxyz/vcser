...
function FallbackIcon({ className }: { className?: string }) {
  return (
    <div className={["flex items-center justify-center rounded-md border border-slate-200 bg-slate-50 text-slate-400 shrink-0", className].join(" ")}>
...
export function EditorIdentity({ editor, mode = "compact", className }: EditorIdentityProps) {
  switch (mode) {
    case EDITOR_IDENTITY_MODE.CARD:
      return (
        <div className={["flex items-center gap-3", className].join(" ")}>
          ...
          <span className="text-xl font-semibold leading-7 text-slate-950">{editor.displayName ?? editor.name}</span>
        </div>
      );
    ...
    case EDITOR_IDENTITY_MODE.ICON:
      return <EditorImg editor={editor} className={["h-11 w-11 rounded-lg", className].filter(Boolean).join(" ")} />;
